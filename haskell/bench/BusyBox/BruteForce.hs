...
-- | a record to store an analysis. I've hardcoded these particular to busybox, if the analysis done in type chef changes these will also have to change
...
readPropFile f = do putStrLn $ "Reading FP: " ++ f
                    txtProblems <- T.lines <$> TIO.readFile f
