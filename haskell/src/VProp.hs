...
import Control.Monad (ap, liftM, liftM3, liftM2)
...
data VProp d a = Ref a                           -- ^ A literal result of a choice
               | Op1 (Op1 d a)                   -- ^ Unary terms
               | Op2 (Op2 d a)                   -- ^ Binary Terms
               | Chc d  (VProp d a) (VProp d a)  -- ^ A Choice in Dimension d
            deriving Eq

data Op1 d a = Neg (VProp d a)                   -- ^ Negation of a term
             deriving Eq

data Op2 d a = And    (VProp d a) (VProp d a)    -- ^ Logical And
             | Or     (VProp d a) (VProp d a)    -- ^ Logical Or
             | Impl   (VProp d a) (VProp d a)    -- ^ Logical implication
             | BiImpl (VProp d a) (VProp d a)    -- ^ BiImplication
             deriving Eq
...
instance (Show a, Show b) => Show (Op1 a b) where
  show (Neg a) = "-" ++ show a

instance (Show a, Show b) => Show (Op2 a b) where
...
instance Bifunctor VProp where
  ...
  bimap f g (Op1 a)      = Op1 $ bimap f g a
  bimap f g (Op2 a)      = Op2 $ bimap f g a

instance Bifunctor Op1 where
  bimap f g (Neg a) = Neg $ bimap f g a

instance Bifunctor Op2 where
  bimap f g (And l r) = And (bimap f g l) (bimap f g r)
  bimap f g (Or l r) = Or (bimap f g l) (bimap f g r)
  bimap f g (Impl l r) = Impl (bimap f g l) (bimap f g r)
  bimap f g (BiImpl l r) = BiImpl (bimap f g l) (bimap f g r)

instance Functor (VProp d) where
  fmap = bimap id

instance Functor (Op1 d) where
  fmap = bimap id

instance Functor (Op2 d) where
  fmap = bimap id
...
instance Bifoldable Op1 where
  bifoldr f g acc (Neg a)      = bifoldr f g acc a

instance Bifoldable Op2 where
...
instance Bifoldable VProp where
  ...
  bifoldr f g acc (Op1 a)      = bifoldr f g acc a
  bifoldr f g acc (Op2 a)      = bifoldr f g acc a
...
instance Bitraversable Op1 where
  bitraverse f g (Neg a)      = Neg <$> bitraverse f g a

instance Bitraversable Op2 where
...
instance Bitraversable VProp where
  ...
  bitraverse f g (Op1 a)      = Op1 <$> bitraverse f g a
  bitraverse f g (Op2 a)      = Op2 <$> bitraverse f g a
  bitraverse f g (Chc d l r) = Chc <$>
                               f d <*>
                               bitraverse f g l <*> bitraverse f g r

-- instance Monad (Op1 d) where
--   Neg x >>= f = Neg $ x >>= f

-- instance Monad (Op2 d) where
...
-- ex = Op2 $ And
--   (Op2 $ BiImpl (Ref "a") (Ref "b"))
--   (Op2 $ Or
--    (Impl (Ref "b") (Ref "c"))
--    (Op2 $ And (Ref "d") (Chc "d" (Ref "e") (Ref "e"))))
