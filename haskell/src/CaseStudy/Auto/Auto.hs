...
type Annot a = S.State ((M.Map V.Dim (EvoContext a)), Integer)
...
autoToVSat :: Show a => AutoLang a -> Annot a (V.VProp a a)
autoToVSat (AutoLit a) = return $ V.LitB a
autoToVSat (Ctx op aexpr boolexpr) =
  do (dims, i) <- S.get
     let newDim = V.Dim $ "D_" ++ show i
         retExpr = flip (V.ChcB newDim) (V.LitB True) <$> (autoToVSat boolexpr)
     case (newDim `M.lookup` dims) of
       Nothing -> do -- this evo context has not been seen
         S.put ((M.insert newDim [(op, aexpr)] dims), succ i)
         retExpr
       (Just _) -> do -- this evo context has been seen
         S.modify (first $ M.insertWith (++) newDim [(op, aexpr)])
         retExpr


  -- V.ChcB "AA" (autoToVSat b) (V.LitB True)
autoToVSat (AutoRef a) = return $ V.RefB a
autoToVSat (AutoNot a) = V.OpB V.Not <$> autoToVSat a
autoToVSat (BBinary And l r) = V.Opn V.And <$> traverse autoToVSat [l,r]
autoToVSat (BBinary Or l r) = V.Opn V.Or <$> traverse autoToVSat [l,r]
autoToVSat (BBinary op l r) = V.OpBB (dispatch op) <$> (autoToVSat l) <*> (autoToVSat r)
autoToVSat (RBinary op l r) = return $ V.OpIB (dispatch' op) (autoToVSat' l) (autoToVSat' r)
