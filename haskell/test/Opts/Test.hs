...
--   do prop <- genVProp :: IO (VProp Var Var)
--      print "--------------\n"
--      print prop
--      print $ atomize prop
--      print "--------------\n"
...
atomTst = H.testCase "Atomization drives choices as far down as possible" $
  ...
  where
    prop :: VProp Var Var
    prop = true &&& bChc "AA"
           (1 + iChc "BB" (1 + 2) 2 .< (5 :: VIExpr Var))
           (2 + iChc "CC" 3 4 .> (5 :: VIExpr Var))
...
atomTst3 = H.testCase "Failure upon nested boolean operators" $
  ...
  where
    prop :: VProp Var Var
    prop = true &&& bChc "AA"
           (false ||| iChc "BB" 1 2 .< (5 :: VIExpr Var))
           (2 + iChc "CC" 3 4 .> (5 :: VIExpr Var))

driveTst = H.testCase "Choices are driven down over like operators" $
           atomize prop `compare` resProp H.@?= EQ
  where prop :: VProp Var Var
        prop = (5 :: VIExpr Var) .>= iChc "A" (1 + 2) (3 + 4)
        resProp = (5 ::VIExpr Var) .>= (iChc "A" 1 3) + (iChc "A" 2 4)
